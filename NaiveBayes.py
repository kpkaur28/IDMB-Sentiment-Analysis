...
class NaiveBayes:
    ...
    def __init__(self):
        ...
        self.pos_words = collections.defaultdict(lambda: 0)
        self.neg_words = collections.defaultdict(lambda: 0)
        self.pos_unique = set()
        self.neg_unique = set()
        self.unique = set()
        ...
        self.pos_docs_wordscount = collections.defaultdict(lambda: 0)
        self.neg_docs_wordscount = collections.defaultdict(lambda: 0)
	...
	#.806500
        ...
        if (self.naiveBayesBool == False and self.bestModel != True):
            ...
            prob_pos = math.log(self.pos_docs_count) - (self.total_doc_count)
            prob_neg = math.log(self.neg_docs_count) - (self.total_doc_count)

            for w in words:
                pos_total = (math.log(self.pos_words[w] + 1)) - (math.log(self.pos_words_count))
                neg_total = (math.log(self.neg_words[w] + 1)) - (math.log(self.neg_words_count))
                prob_pos += pos_total
                prob_neg += neg_total
        ...
        if (self.naiveBayesBool == True and self.bestModel != True):
             ...
             for w in words:
                if w not in seen:
                    pos_total = (math.log(self.pos_docs_wordscount[w] + alpha)) - (math.log(self.pos_words_count + (alpha * vocab)))
                    neg_total = (math.log(self.neg_docs_wordscount[w] + alpha)) - (math.log(self.neg_words_count + (alpha * vocab)))
                    prob_pos += pos_total
                    prob_neg += neg_total
		    seen.add(w)
	...
	#.841000
        if self.bestModel == True:
            ...
            vocab = len(self.pos_unique) + len(self.neg_unique) + alpha
    ...
    def addDocument(self, classifier, words):
        ...
        for w in words:
            if classifier == 'pos':
                ...
                self.pos_docs_wordscount[w] += 1
                self.pos_docs_count += 1
                self.pos_unique.add(w)

            if classifier == 'neg':
                self.neg_words[w] += 1
                self.neg_words_count += 1
                self.neg_docs_wordscount[w] += 1
                self.neg_docs_count += 1
                self.neg_unique.add(w)
